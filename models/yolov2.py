...
from models.loss import build_target, yolo_loss
...
class ReorgLayer(nn.Module):
    ...
    def forward(self, x):
        B, C, H, W = x.data.size()  # B:1, C:64, H:26, W:26
        ws = self.stride    # ws:2
        hs = self.stride    # hs:2
        x = x.view(B, C, int(H / hs), hs, int(W / ws), ws).transpose(3, 4).contiguous() # (1, 64, 26, 26)->(1, 64, 13, 13, 2, 2)
        x = x.view(B, C, int(H / hs * W / ws), hs * ws).transpose(2, 3).contiguous()    # (1, 64, 13, 13, 2, 2)->(1, 64, 4, 169)
        x = x.view(B, C, hs * ws, int(H / hs), int(W / ws)).transpose(1, 2).contiguous()    # (1, 64, 4, 169)->(1, 4, 64, 13, 13)
        x = x.view(B, hs * ws * C, int(H / hs), int(W / ws))    # (1, 4, 64, 13, 13)->(1, 256, 13, 13)
        return x
...
class Yolov2(nn.Module):
    ...
    def forward(self, x, gt_boxes=None, gt_classes=None, num_boxes=None, training=False):
        ...
        """
        x = self.conv1(x)   # (1, 3, 416, 416) -> (1, 512, 26, 26)
        shortcut = self.reorg(self.downsampler(x))  # (1, 512, 26, 26)->(1, 64, 26, 26)->(1, 256, 13, 13)
        x = self.conv2(x)   # (1, 512, 26, 26)->(1, 1024, 13, 13)
        x = self.conv3(x)   # (1, 1024, 13, 13)->(1, 1024, 13, 13)
        x = torch.cat([shortcut, x], dim=1) # (1, 1024, 13, 13)->(1, 1280, 13, 13)
        out = self.conv4(x) # (1, 1280, 13, 13)->(1, 125, 13, 13)
